class ApplicationController < ActionController::Base
  ...
  secret_path = File.join(Rails.root, "config/secret.txt")
  ...
  def add_crumb_show_action
    add_crumb(t("show", :scope => [:controller, controller_name]), view_context.action_path(:show, :id => params[:id]))
  end

  def add_crumb_list_action
    path = view_context.back_list_path.dup
    tag = path.delete(:tag)
    add_crumb(message_of_user(path[:action], :scope => [:controller, controller_name]), path)
    add_crumb(ERB::Util.h(tag), view_context.back_list_path) if tag
  end
  ...
  def add_crumb_create_action
    add_crumb(t("create", :scope => [:controller, controller_name]), view_context.action_path(:new))
  end
  
  def add_crumb_update_action
    add_crumb(t("update", :scope => [:controller, controller_name]), view_context.action_path(:edit, :id => params[:id]))
  end
